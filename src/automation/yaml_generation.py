...
def generate_hv_ioda_inventory_config(inventory_info, start_time, end_time, work_dir):
    filename = inventory_info.obs_name + dt.now().strftime("%Y%m%d%H%M%S") + str(random.randint(1, 9999)) + '_obs_meta_hv_ioda.yaml'
    yaml_file_path = os.path.join(PY_CURRENT_DIR, filename)
    if start_time is dt:
        start = start_time.strftime(au.DATESTR_FORMAT)
    else:
    ...
    body = {
        ...
        'ioda_files':[inventory_info.files],
        'work_dir': work_dir,
        'scrub_files': True
    }
    outfile = open(yaml_file_path, 'w')
...
def generate_hv_wod_inventory_config(inventory_info, start_time, end_time, work_dir):
    ...
    body = {
        ...
        'wod_files':[inventory_info.files],
        'work_dir': work_dir,
        'scrub_files': True
    }
    outfile = open(yaml_file_path, 'w')
