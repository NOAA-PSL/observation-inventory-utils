...
def generate_obs_inv_config(inventory_info, start_time, end_time):
    filename = inventory_info.obs_name + dt.datetime.now().strftime("%Y%m%d%H%M%S") + str(random.randint(1, 9999)) + '_obs_inv_config.yaml'
...
def generate_nceplibs_sinv_inventory_config(inventory_info, start_time, end_time, work_dir):
    filename = inventory_info.obs_name + dt.datetime.now().strftime("%Y%m%d%H%M%S") + str(random.randint(1, 9999)) + '_obs_meta_sinv.yaml'
    yaml_file_path = os.path.join(PY_CURRENT_DIR, filename)
    if start_time is dt:
        start = start_time.strftime(au.DATESTR_FORMAT)
    else:
...
def generate_nceplibs_cmpbqm_inventory_config(inventory_info, start_time, end_time, work_dir):
    filename = inventory_info.obs_name + dt.datetime.now().strftime("%Y%m%d%H%M%S") + str(random.randint(1, 9999)) + '_obs_meta_cmpbqm.yaml'
    yaml_file_path = os.path.join(PY_CURRENT_DIR, filename)
    if start_time is dt:
        start = start_time.strftime(au.DATESTR_FORMAT)
    else:
