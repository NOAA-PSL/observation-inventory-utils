...
ax.set_title(f'accurate as of {datetime.now().strftime("%m/%d/%Y %H:%M:%S")}')
# ax2 = ax.twinx()
# ax2.set_yticks(step/2+step*np.arange(counter))
# ax2.set_yticklabels(directory_labels)
# ax2.set_ylim([0, height])

plt.suptitle("Inventory of Clean Bucket Atmosphere Sensors by Satellite")
...
plt.savefig(fnout, bbox_inches='tight', pad_inches=0.2)
mysql_conn.close()
