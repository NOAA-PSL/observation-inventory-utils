...
#parameters
daterange=[date(1970,1,1), date(date.today().year+1,1,1)]
...
unique_sensor_dir_version = db_frame[['source_dir', 'sensor', 'ioda_version']].value_counts().reset_index(name='count').sort_values(by = ['sensor', 'source_dir', 'ioda_version'], ascending=[False, False, False])
step=0.05
height=step*len(unique_sensor_dir_version)
...
plt.title("Inventory of NNJA IODA Ice files")
...
for index, row in unique_sensor_dir_version.iterrows():
    ...
    dftmp = select_sensor_dir_version(row['sensor'], row['source_dir'], row['ioda_version'], db_frame)
...
ax.set_yticklabels(sensor_dir_ver_labels)
...
file_name = "ice_ver_line_observations_inventory_.png"
if args.dev:
    file_name = "ice_ver_line_observations_inventory_" + datetime.now().strftime("%Y%m%d%H%M%S") + ".png"
