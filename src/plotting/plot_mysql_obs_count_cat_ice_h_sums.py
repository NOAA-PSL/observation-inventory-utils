...
# Define the specific categories to include in the total
total_categories = ['icec_glore_nh', 'icec_glore_sh']

# Filter the DataFrame to only include the specified categories
total_df = grouped_df[grouped_df['category'].isin(total_categories)]

# Calculate the total of the rolling averages and apply a final smoothing
total_series = total_df.groupby('date_only')['rolling_avg'].sum()
smoothed_total = total_series.rolling(window=args.window, min_periods=1).mean().reset_index()

# Plot the total line
ax.plot(smoothed_total['date_only'], smoothed_total['rolling_avg'], label='Ice Concentration Total', color='black', linestyle='--', linewidth=2)
...
file_name = f"ice_time_series_combo_v{args.ioda_version}_sum_h_{args.window}_days.png"
if args.dev:
    file_name = f"ice_time_series_combo_v{args.ioda_version}_sum_h_{args.window}_days_" + datetime.now().strftime("%Y%m%d%H%M%S") + ".png"
